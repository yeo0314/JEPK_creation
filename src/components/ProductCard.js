...
const ProductCard = ({ product }) => {
  ...
  // Précharger les images pour debug: log success / failure
  useEffect(() => {
    if (!images || images.length === 0) return;
    images.forEach((src) => {
      const img = new Image();
      img.onload = () => console.log('[IMG LOAD]', src);
      img.onerror = () => console.error('[IMG ERR]', src);
      img.src = src;
    });
  }, [images]);
  ...
  return (
    <div className="bg-white rounded-2xl shadow-lg overflow-hidden transform hover:scale-105 transition duration-300 group">
      {/* Image Section with Gallery - Cliquable */}
      <div 
        onClick={() => navigate(`/product/${product.id}`)}
        className="h-48 bg-gradient-to-br from-purple-200 to-pink-200 relative overflow-hidden group cursor-pointer"
      >
        {/* Overlay "Voir détails" au survol */}
        <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center z-10">
          <div className="bg-white text-purple-600 px-4 py-2 rounded-full font-semibold flex items-center space-x-2">
